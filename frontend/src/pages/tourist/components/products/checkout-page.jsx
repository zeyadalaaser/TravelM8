...
export default function CheckoutPage() {
    ...
    const fetchWalletBalance = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:5001/api/user/wallet-balance",
          {
            headers: {
  ...
  const handleAddAddress = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(
        "http://localhost:5001/api/choose-delivery-address",
        { newAddress },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSelectAddress = async (addressId) => {
    try {
      ...
      toast.error(
        error.message || "Failed to select address. Please try again."
      );
    }
  };
  ...
  const handlePayment = async (paymentIntentId = null) => {
    try {
      ...
      navigate("/tourist-page?type=products&payment=success", {
              ...
              <ul className="mt-2 divide-y divide-gray-200">
                {cart.map((item) => (
                  <li key={item._id} className="py-2 flex justify-between">
                    ...
                    <span>
                      {(item.price * item.quantity).formatCurrency(currency)}
                    </span>
                  </li>
                ))}
              </ul>
              ...
              <RadioGroup onValueChange={setPaymentMethod}>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="wallet">
                    Wallet (Balance: {walletBalance.formatCurrency(currency)})
                  </Label>
                </div>
              </RadioGroup>
            </div>
